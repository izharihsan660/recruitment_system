...
export default function EntityIndex({ entities }: { entities: Paginated<Entity> }): JSX.Element {
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-lg font-semibold">Entitas (PT)</h1>}>
            ...
            <Card className="overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                        {rows(entities).map((entity) => <tr key={entity.id}><td className="p-4 font-medium">{entity.name}</td><td className="p-4">{entity.short_name}</td><td className="p-4"><Badge tone={entity.is_active ? 'green' : 'slate'}>{entity.is_active ? 'Aktif' : 'Nonaktif'}</Badge></td><td className="space-x-2 p-4 text-right"><Button variant="secondary" onClick={() => startEdit(entity)}>Edit</Button><Button variant="ghost" onClick={() => toggle(entity)}>Toggle Aktif</Button><Button variant="destructive" size="sm" onClick={() => { if (confirm('Hapus data ini?')) { router.delete(`/admin/entities/${entity.id}`); } }}>Hapus</Button></td></tr>)}
                    </tbody>
                </table>
            ...
            {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><Card className="w-full max-w-lg p-6"><h2 className="mb-4 text-lg font-semibold">{editing ? 'Edit Entitas' : 'Tambah Entitas'}</h2><form onSubmit={submit} className="space-y-4"><div><FormLabel>Nama</FormLabel><TextInput value={form.data.name} onChange={(event) => form.setData('name', event.target.value)} /><FieldError message={form.errors.name} /></div><div><FormLabel>Singkatan</FormLabel><TextInput value={form.data.short_name} onChange={(event) => form.setData('short_name', event.target.value)} /><FieldError message={form.errors.short_name} /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.data.is_active} onChange={(event) => form.setData('is_active', event.target.checked)} /> Status aktif</label><div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Batal</Button><Button type="submit" disabled={form.processing}>{form.processing ? 'Menyimpan...' : 'Simpan'}</Button></div></form></Card></div>}
        </AuthenticatedLayout>
    );
}
