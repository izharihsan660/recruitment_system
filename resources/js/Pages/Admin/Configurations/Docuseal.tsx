...
export default function Docuseal({ docusealConfigs }: { docusealConfigs: DocusealConfig[] }): JSX.Element {
    ...
    function submit(event: FormEvent): void { event.preventDefault(); current ? router.put(`/admin/docuseal-configs/${current.id}`, form.data, {}) : router.post('/admin/docuseal-configs', form.data, {}); }
    function test(): void { if (!current) { setResult('Simpan konfigurasi dulu sebelum test koneksi.'); return; } router.post(`/admin/docuseal-configs/${current.id}/test-connection`, {}, { onSuccess: () => setResult('Koneksi DocuSeal berhasil.'), onError: () => setResult('Koneksi DocuSeal gagal. Periksa API URL dan API Key.') }); }
    return <AuthenticatedLayout header={<h1 className="text-lg font-semibold">DocuSeal</h1>}><Head title="DocuSeal" /><PageHeader title="DocuSeal" description="Konfigurasi DocuSeal Cloud untuk e-sign Offering dan PKWT." /><Card className="p-6"><form onSubmit={submit} className="grid gap-4 md:grid-cols-2"><div><FormLabel>API URL</FormLabel><TextInput value={form.data.api_url} onChange={(e) => form.setData('api_url', e.target.value)} /></div><div><FormLabel>API Key</FormLabel><TextInput type="password" value={form.data.api_key} onChange={(e) => form.setData('api_key', e.target.value)} placeholder={current?.has_api_key ? 'Masked' : 'Masukkan API Key'} /></div><div><FormLabel>Webhook Secret</FormLabel><TextInput type="password" value={form.data.webhook_secret} onChange={(e) => form.setData('webhook_secret', e.target.value)} placeholder={current?.has_webhook_secret ? 'Masked' : 'Opsional'} /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.data.is_active} onChange={(e) => form.setData('is_active', e.target.checked)} /> Status aktif</label><div className="flex justify-end gap-2 md:col-span-2"><Button type="button" variant="secondary" onClick={test}>Test Koneksi</Button><Button type="submit" disabled={form.processing}>{form.processing ? 'Menyimpan...' : 'Simpan'}</Button></div>{result && <div className="md:col-span-2"><Badge tone={result.includes('berhasil') ? 'green' : 'red'}>{result}</Badge></div>}</form></Card></AuthenticatedLayout>;
}
