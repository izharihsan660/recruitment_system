import { Badge, Button, Card, FormLabel, PageHeader, SelectInput, TextArea } from '@/Components/shared/ui';
import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout';
import { EmailIntake, formatDate, humanize, JobPosting } from '@/lib/recruitment';
import { PageProps } from '@/types';
import { Head, router, useForm, usePage } from '@inertiajs/react';

export default function EmailIntakeShow({ email, jobPostings }: { email: EmailIntake; jobPostings: JobPosting[] }): JSX.Element {
    const form = useForm({ job_posting_id: '', consent: false, notes: '', reason: '' });
    const { errors } = usePage<PageProps>().props;

    return (
        <AuthenticatedLayout header={<h1 className="text-lg font-semibold">Detail Email</h1>}>
            <Head title="Detail Email" />
            <PageHeader title={email.subject ?? 'Email'} description={`${email.sender_email ?? '-'} • ${formatDate(email.received_at)}`} />
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
                ...
                <Card className="p-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <FormLabel>Assign ke Lowongan</FormLabel>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <Button variant="danger" onClick={() => router.post(`/hr/email-intake/${email.id}/reject`, { reason: form.data.reason })}>Reject</Button>
                            <Button variant="secondary" onClick={() => router.post(`/hr/email-intake/${email.id}/ignore`)}>Ignore</Button>
                            <Button variant="secondary" onClick={() => router.post(`/hr/email-intake/${email.id}/spam`)}>Spam</Button>
                        </div>
                    </div>
                </Card>
            </div>
