...
export default function Screening({ application, screening }: { application: ApplicationItem; screening?: Partial<ScreeningData> | null }): JSX.Element {
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-lg font-semibold">Screening Kandidat</h1>}>
            ...
            <PageHeader title="Screening Kandidat" description={`${application.candidate?.name ?? 'Kandidat'} - ${application.job_posting?.position_name ?? ''}`} actions={<Link className="text-sm text-blue-600" href="/pipeline">Kembali</Link>} />
            <Card className="p-6">
