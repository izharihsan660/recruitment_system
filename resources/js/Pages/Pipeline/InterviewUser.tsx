...
export default function InterviewUser({ application, interview, interviewers, canSchedule, canScorecard }: { application: ApplicationItem; interview?: InterviewRecord | null; interviewers: BasicUser[] | { data: BasicUser[] }; canSchedule: boolean; canScorecard: boolean }): JSX.Element {
    ...
    return (
        <AuthenticatedLayout header={<h1 className="text-lg font-semibold">Interview User</h1>}>
            ...
            <PageHeader title="Interview User" description={`${application.candidate?.name ?? 'Kandidat'} - ${application.job_posting?.position_name ?? ''}`} actions={<Link className="text-sm text-blue-600" href="/pipeline">Kembali</Link>} />
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="p-6">
                    ...
                    <form onSubmit={(event) => { event.preventDefault(); const method = interview?.id ? scheduleForm.put : scheduleForm.post; method(`/hr/interview-user/${application.id}/${interview?.id ? 'reschedule' : 'schedule'}`); }} className="space-y-4">
                        <div><FormLabel>Tanggal & Waktu</FormLabel><TextInput type="datetime-local" disabled={!canSchedule} value={scheduleForm.data.scheduled_at} onChange={(event) => scheduleForm.setData('scheduled_at', event.target.value)} /><FieldError message={scheduleForm.errors.scheduled_at} /></div>
                        <div><FormLabel>Lokasi</FormLabel><TextInput disabled={!canSchedule} value={scheduleForm.data.location} onChange={(event) => scheduleForm.setData('location', event.target.value)} /><FieldError message={scheduleForm.errors.location} /></div>
                        <div><FormLabel>Interviewer</FormLabel><SelectInput disabled={!canSchedule} value={scheduleForm.data.interviewer_id} onChange={(event) => scheduleForm.setData('interviewer_id', event.target.value)}>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</SelectInput></div>
                ...
                <Card className="p-6">
                    ...
                    <form onSubmit={(event) => { event.preventDefault(); scoreForm.post(`/hr/interview-user/${application.id}/scorecard`); }} className="space-y-4">
                        <ScoreRating label="Kemampuan Teknis" value={scoreForm.data.score_technical} onChange={(value) => setScore('score_technical', value)} readOnly={!canScorecard} />
                        <ScoreRating label="Pengalaman Kerja" value={scoreForm.data.score_experience} onChange={(value) => setScore('score_experience', value)} readOnly={!canScorecard} />
                        <ScoreRating label="Problem Solving" value={scoreForm.data.score_problem_solving} onChange={(value) => setScore('score_problem_solving', value)} readOnly={!canScorecard} />
                        <ScoreRating label="Kesesuaian dengan Tim" value={scoreForm.data.score_team_fit} onChange={(value) => setScore('score_team_fit', value)} readOnly={!canScorecard} />
                        <div><FormLabel>Rekomendasi</FormLabel><SelectInput disabled={!canScorecard} value={scoreForm.data.recommendation} onChange={(event) => scoreForm.setData('recommendation', event.target.value as never)}><option value="accepted">Diterima</option><option value="considered">Dipertimbangkan</option><option value="rejected">Ditolak</option></SelectInput></div>
                        {scoreForm.data.recommendation === 'rejected' && <div><FormLabel>Alasan Penolakan</FormLabel><TextArea disabled={!canScorecard} rows={3} value={scoreForm.data.rejection_reason} onChange={(event) => scoreForm.setData('rejection_reason', event.target.value)} /><FieldError message={scoreForm.errors.rejection_reason} /></div>}
                        <div><FormLabel>Catatan</FormLabel><TextArea disabled={!canScorecard} rows={3} value={scoreForm.data.notes} onChange={(event) => scoreForm.setData('notes', event.target.value)} /></div>
